...
- **Business spec compliance:** ✓ — R1–R7 all verified: title/meta updated, `<h1>` added, all labels in French, `<details>`/`<summary>` wrapping Suspense block (no `open` attribute), `/mentions-legales` route accessible via footer link, Markdown asset present with no `jeremielitzler.fr` references.
